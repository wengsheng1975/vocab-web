...
// 独立模式：启动时创建与 demo 端点一致的测试用户（tester），使用随机密码
if (isStandalone) {
  const db = require('./config/db');
  const bcrypt = require('bcryptjs');
  const demoUsername = 'tester';
  const existing = db.prepare('SELECT id FROM users WHERE username = ?').get(demoUsername);
  if (!existing) {
    const randomPassword = crypto.randomBytes(16).toString('hex');
    const hash = bcrypt.hashSync(randomPassword, 10);
    db.prepare('INSERT INTO users (username, email, password_hash) VALUES (?, ?, ?)').run(demoUsername, 'tester@demo.com', hash);
    console.log('独立模式：已创建测试用户 tester（通过「一键体验」登录）');
  }
}
